...
在 GitHub Pages 仍采用当前 workflow 发布模式的前提下，开发版发版需要遵守下面这条操作规约：

- 每个 `v*-dev*` tag 都必须对应一个新的源码提交。
- 不要对同一提交重复创建多个开发版 tag，并期待公开 `dev.xml` 自动切到最新版本。
- 如果这次只是重发或补发，没有实际代码变更，也需要先创建一个空提交，再创建新的开发版 tag。

这条规约只是当前发布链路的临时绕行方案，不等同于从根本上修复 Pages 在“同一提交多 tag”场景下的 appcast 刷新问题。
