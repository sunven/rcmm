...
```bash
TRANSITION=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" workstream list --raw)
fi
```
...
```bash
node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
