...
@/Users/sunven/github/rcmm/.codex/get-shit-done/references/agent-contracts.md
@/Users/sunven/github/rcmm/.codex/get-shit-done/references/context-budget.md
@/Users/sunven/github/rcmm/.codex/get-shit-done/references/gates.md
</required_reading>
...
```bash
INIT=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" init execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-executor 2>/dev/null)
```
...
```bash
USE_WORKTREES=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.use_worktrees 2>/dev/null || echo "true")
```
...
```bash
CONTEXT_WINDOW=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" config-get context_window 2>/dev/null || echo "200000")
```
...
if [[ ! "{{GSD_ARGS}}" =~ --auto ]]; then
  node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/Users/sunven/github/rcmm/.codex/get-shit-done/workflows/execute-plan.md` **inline**
...
```bash
node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Task(
     ...
     prompt="
       ...
       <execution_context>
       @/Users/sunven/github/rcmm/.codex/get-shit-done/workflows/execute-plan.md
       @/Users/sunven/github/rcmm/.codex/get-shit-done/templates/summary.md
       @/Users/sunven/github/rcmm/.codex/get-shit-done/references/checkpoints.md
       @/Users/sunven/github/rcmm/.codex/get-shit-done/references/tdd.md
       </execution_context>
   ...
   for WT in $WORKTREES; do
     ...
     if [ -n "$WT_BRANCH" ] && [ "$WT_BRANCH" != "HEAD" ]; then
       ...
       if ! git diff --quiet .planning/STATE.md .planning/ROADMAP.md 2>/dev/null || \
         ...
         COMMIT_DOCS=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" config-get commit_docs 2>/dev/null || echo "true")
...
5.6. **Post-wave shared artifact update (worktree mode only):**
   ...
   for PLAN_ID in ${WAVE_PLAN_IDS}; do
     node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE_NUMBER}" "${PLAN_ID}" completed
   done
...
7b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_CHAIN=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
**Security gate check:**
```bash
SECURITY_CFG=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.security_enforcement --raw 2>/dev/null || echo "true")
...
```bash
POST_PLAN_INDEX=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
**Config gate:**
```bash
CODE_REVIEW_ENABLED=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.code_review 2>/dev/null || echo "true")
```
...
```bash
PARENT_INFO=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
SCHEMA_DRIFT=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" verify schema-drift "${PHASE_NUMBER}" 2>/dev/null)
```
...
```bash
VERIFIER_SKILLS=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-verifier 2>/dev/null)
```
...
Commit the file:
```bash
node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
```bash
COMPLETION=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
```bash
node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
**Check config gate:**
```bash
GL_ENABLED=$(node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" config-get features.global_learnings --raw 2>/dev/null || echo "false")
```
...
```bash
node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" learnings copy 2>/dev/null || echo "⚠ Learnings copy failed — continuing"
```
...
```bash
node "/Users/sunven/github/rcmm/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
Read and follow `/Users/sunven/github/rcmm/.codex/get-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
