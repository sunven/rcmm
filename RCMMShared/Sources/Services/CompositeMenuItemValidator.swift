...
public enum CompositeMenuItemValidator: Sendable {
    ...
    private static func validateStep(_ step: CompositeCommandStep) -> [CompositeValidationIssue] {
        ...
        for pattern in dangerousPatterns(in: step.commandTemplate) {
            issues.append(
                issue(
    ...
    private static func dangerousPatterns(in commandTemplate: String) -> [String] {
        let normalized = commandTemplate
            .lowercased()
